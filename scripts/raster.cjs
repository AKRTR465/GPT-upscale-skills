...
class DiskRaster {
  static create(filename, width, height) {
    assert(Number.isInteger(width) && Number.isInteger(height) && width > 0 && height > 0 && width * height <= 256000000, 'Invalid raster dimensions');
    const rowsPerStrip = 64, strips = Math.ceil(height / rowsPerStrip), count = 11;
    ...
    try { fs.writeSync(fd,header); fs.ftruncateSync(fd,pixelOffset+width*height*4); }
  ...
  async fillFrom(source, origin={left:0,top:0}) { for(const r of windows(this.width,this.height)) { const pixels=await sharp(source,{limitInputPixels:256000000}).extract({...r,left:r.left+origin.left,top:r.top+origin.top}).ensureAlpha().raw().toBuffer(); this.write(r,pixels); } }
