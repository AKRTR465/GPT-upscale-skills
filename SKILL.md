...
- Keep the user's aspect ratio, destination, output formats, identity constraints, and existing authorization. Watermark removal defaults to **off**. Enable it only for an explicit request such as "超分并去水印" or "开启去水印"; "保留水印/关闭去水印" disables it. Preserve marks when removal is off, including in redraw prompts. Enabling watermark cleanup does not authorize unrelated object/text removal.
...
## Optional watermark cleanup: off by default

Read [the watermark workflow](references/watermark-removal.md) **only when cleanup is enabled**. Inspect and mask the exact overlay, reconstruct the local background, register/color-match using unchanged surroundings, composite only inside the mask, and verify locality. Feed the reviewed lossless clean source into the regular pipeline so the original overlay cannot leak back from the base or overlapping tiles.

`plan` and `prepare` accept `--watermark-removal off|on` (default `off`). With `on`, supply `--removal-record FILE` for a completed cleanup assessment: `cleaned`, `not-found`, or `partial`. The CLI checks source/result hashes, dimensions and record consistency and carries the record into the manifest and export; it does not itself detect or erase marks. Existing task authorization suffices; recording QA is not another permission step. Keep the switch scoped to the requested image/batch, with explicit per-image exceptions. To change the mode after preparation, create a new job from the appropriate source; toggling a JSON field cannot undo a cleaned base.
