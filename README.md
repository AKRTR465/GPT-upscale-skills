...
支持上限为 **256MP 画布、每任务 1000 个实际编辑块**，后者包含独立细节区域的子块。16K 方图为 15360×15360，约 235.9MP。大图合成和校验应串行执行，并按规划报告预留临时磁盘空间。旧任务的坐标保持不变；若需向超过 1K 的旧区域导入新结果，应从原图建立新任务。
...
验证命令：`npm run check`、`npm test`。功能测试覆盖自动分块、区域组、旧任务、文件支持的合成和全像素 SVG 校验；独立 16K 验收覆盖横图、4:3、方图及竖图，并以单进程峰值内存不超过 4GiB 为验收标准。测试使用合成图，不调用模型，也不评价模型重绘质量。
