...
**可选去水印默认关闭。** 说“超分并去水印”即可在本次任务开启，说“保留水印/关闭去水印”则关闭。开启后先局部背景补绘、蒙版贴回和蒙版外像素检查，再以清理后的无损图进入超分，避免水印从底图或拼接处回流。CLI 使用 `--watermark-removal off|on`，开启时附 `--removal-record`；完整步骤与记录格式见 [去水印工作流](references/watermark-removal.md)。脚本校验并记录此阶段，不会自行调用图像生成服务。
